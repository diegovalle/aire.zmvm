...
We can also recreate the [Get Started](../aire.zmvm.html) article, but instead of downloading the data in IMECAS we can download it in the original units and then use the `convert_to_imeca` function to recreate the pollution map.
...
get_data_roll <- function(pollutant, mxc, ave) {
  # Make sure to download data for May 2017
...
qmplot(x, y, data = geography.grd, geom = "blank", 
  ...
  ggtitle("Pollution in Mexico City (Midnight May 31, 2017)")
```
