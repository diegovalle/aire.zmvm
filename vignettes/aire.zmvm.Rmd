...
This document introduces you to tools for downloading data for each of the pollution, wind and temperature measuring stations or the maximum pollutant values by geographical zones in the Zona Metropolitana del Valle de México (greater Mexico City).

## Core Functions

The basic set of functions provided by the package:

* ```get_station_imeca``` download data for each station in IMECAs
* ```get_station_data``` and ```get_station_single_month``` to download pollution data for each of the pollution (and wind and temperature) measuring stations in the original units.
...
of time returned by the function in the data.frame above. That's because the data returned by all package functions comes in a format that is not affected by daylight savings time. To covert to local Mexico City time:
...
get_grid <- function(station_vec, pixels = 20) {
...
grid <- get_grid(reporting_stations, 50)
plot(grid)
