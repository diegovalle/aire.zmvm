...
![Mexico City Smog](reference/figures/pano-mxc.jpg) 

<br>

**aire.zmvm** is an R package for downloading air quality data from the Mexico City metropolitan area. This package provides real-time, daily maximum, minimum, and hourly average data for pollution measuring stations and geographical zones within the *Zona Metropolitana del Valle de México* (Greater Mexico City). It also includes station locations and a specialized function for inverse distance weighting adjusted for wind direction.
...
library(aire.zmvm)
```
*Note: The version on CRAN may not include the latest updates available on GitHub.*


## Usage

This example creates a time series of ozone levels (in ppb) from 2009 to 2018 and highlights Phase I smog alerts.
...
```{r index-contingencias, fig.dim=c(11,6), warning=FALSE}
ggplot(max_daily_df,
       aes(x = x, y = y)) + 
