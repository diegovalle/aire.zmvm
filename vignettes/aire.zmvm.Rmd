...
Because of limitations with the website [http://www.aire.cdmx.gob.mx/](http://www.aire.cdmx.gob.mx/default.php)
...
pollutant reach an hourly average of 151 IMECA points or the Mexico City government can declare a phase I smog alert and order cars off the streets and place limits on polluting industries.

On May 20, 2017 a phase I smog alert was declared. We'll make a plot of pollution levels during the alert. First we load the packages we need to run this vignette and download the pollution data:
...
of time returned by the function in the data.frame above. That's because the data returned by this package functions comes in a format that is not affected by daylight savings time. To convert to local Mexico City time we can use the following function:

```{r}
format(as.POSIXct("2017-05-20 17:00", tz = "UTC+6"), 
...
We will use this data to create a grid covering Mexico City and extrapolate the values by inverse distance weighting from the locations of the pollution measuring stations to each cell in the
grid. 
...
# last available hour of the last available day in February
